locals {
  ...
  cloud_run_environment = {
    ...
    EE_SESSION_SIGNING_KEY_SECRET_REF     = "projects/${var.project_id}/secrets/${local.name_prefix}-session-signing-key/versions/${lookup(var.member_session_secret_versions, "session-signing-key", "latest")}"
    EE_REFRESH_TOKEN_PEPPER_SECRET_REF    = "projects/${var.project_id}/secrets/${local.name_prefix}-refresh-token-pepper/versions/${lookup(var.member_session_secret_versions, "refresh-token-pepper", "latest")}"
    ...
    EE_MEMBER_IDENTITY_PROVIDER           = var.enable_member_session ? "identity_platform" : "mock"
    EE_FIREBASE_PROJECT_ID                = var.enable_member_session ? var.project_id : ""
    ...
    EE_ACCOUNT_CONTROLS_ENABLED           = tostring(var.enable_account_controls)
