...
$runnerPath = Join-Path $PSScriptRoot '..\scripts\Invoke-Week2DatabaseUpdate.ps1'
$tokens = $null
$errors = $null
...
$backup = [pscustomobject]@{
    ...
    description = 'Pre-Week-2 staff and English profile update'; endTime = '2026-09-14T17:30:00Z'
}
...
$entry = [pscustomobject]@{
    ...
    jsonPayload = [pscustomobject]@{ status = 'ok'; applied = @('V0002', 'V0003', 'V0004') }
}
...
$bad.jsonPayload.applied = @('V0001', 'V0002', 'V0003', 'V0004')
Expect-Rejected { Assert-SuccessRecord @($bad) 'migration-job-run1' }
$bad.jsonPayload.applied = @('V0002', 'V0003')
Expect-Rejected { Assert-SuccessRecord @($bad) 'migration-job-run1' }
$bad = Copy-Object $entry
