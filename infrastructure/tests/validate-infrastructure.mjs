...
assert.match(source, /google_monitoring_uptime_check_config" "api_health"\s*\{[\s\S]*?lifecycle\s*\{[\s\S]*?create_before_destroy\s*=\s*true/m, 'a new revision health check must be created before the alert releases the old check');
