...
# A single development update from V0004 to V0005. This does not update the job,
...
$markerPath = Join-Path $controlDirectory 'week2-profile-photo-v0005.json'
$lockPath = Join-Path $controlDirectory 'ee003-development.lock'
$jobAddress = 'google_cloud_run_v2_job.database_migration[0]'
$previousRevision = '9d518fdf0ae08eb7af3d91b6f5203fb86f04c8cf'
$previousDigest = '771b92bdd1e2937c2fe1bd3f8d7f3955687f15a9938560e23ee886538f2eded1'
$pendingIds = @('V0005')
...
function Assert-BackupRecord {
    ...
    $valid = @($Records | Where-Object {
        if ($_.status -cne 'SUCCESSFUL' -or $_.type -cne 'ON_DEMAND' -or
            $_.description -cne 'Pre-EE-013 profile photo table update' -or -not $_.endTime) { return $false }
...
function Assert-SuccessRecord {
    ...
    foreach ($entry in $Logs) {
        ...
        if ($record -and $record.status -ceq 'ok' -and (@($record.applied) -join ',') -ceq 'V0005') {
            $matches += $record
        }
    }
    if ($matches.Count -ne 1) { throw 'The exact V0005 completion record is not yet proven. Do not restart.' }
}
...
function Assert-UpdateMarker {
    ...
    $prior = @($Marker.previousExecutions)
    if ($Marker.projectId -cne $Project -or $Marker.region -cne $Region -or $Marker.jobName -cne $Job -or
        ...
        $Marker.image -cne $Image -or $Marker.planSha256 -cne $PlanChecksum -or
        $prior.Count -ne 2 -or @($prior | Select-Object -Unique).Count -ne 2 -or
        @($prior | Where-Object { $_ -cnotmatch ('^' + [regex]::Escape($Job) + '-[a-z0-9]+$') }).Count -ne 0) {
...
try {
    ...
    if ($ExpectedImage -cnotmatch ('^' + [regex]::Escape($prefix) + '[0-9a-f]{64}$') -or
        $ExpectedImage -ceq "$prefix$previousDigest") { throw 'An approved new immutable migration image is required.' }
    ...
    if ((@($manifest.migrations.id) -join ',') -cne 'V0001,V0002,V0003,V0004,V0005') {
        throw 'This runner is only for the agreed EE-013 profile-photo migration.'
    }
    ...
    $expectedJob = Assert-UpdatePlan -Plan $plan -Image $ExpectedImage -Revision $migrationRevision `
        -PreviousImage "$prefix$previousDigest" -PreviousRevision $previousRevision
    ...
    if ($Action -eq 'Verify') {
        ...
        $priorNames = @($marker.previousExecutions)
        $new = @($history | Where-Object { $priorNames -cnotcontains $_.metadata.name })
        if ($history.Count -ne 3 -or $new.Count -ne 1) {
            throw 'The one additional execution cannot be isolated. Do not restart.'
        }
        ...
        Write-Output 'Verified: V0005 completed in one recorded execution, including database postchecks.'
        return
    }
    if (Test-Path $markerPath) { throw 'A previous start attempt is recorded. Use Verify, never Start again.' }
    $completed = @($history | Where-Object {
        @($_.status.conditions | Where-Object { $_.type -ceq 'Completed' -and $_.status -ceq 'True' }).Count -eq 1 -and
        [int]$_.status.succeededCount -eq 1 -and [int]$_.status.failedCount -eq 0
    })
    if ($history.Count -ne 2 -or $completed.Count -ne 2) {
        throw 'The two expected successful earlier database executions are not proven.'
    }
    $priorUpdate = @()
    foreach ($candidate in $history) {
        $execution = Invoke-GcloudJson -Arguments @('run', 'jobs', 'executions', 'describe', $candidate.metadata.name,
    ...
    if ($priorUpdate.Count -ne 1) {
        throw 'The approved V0002-V0004 execution is not uniquely proven.'
    }
    if ($Action -eq 'Plan') {
        Write-Output 'Validated: dormant job image/source update only; private database; V0005 next. No execution started.'
        return
    }
    if ($ConfirmUpdate -cne 'UPDATE-EE013-DEVELOPMENT-V0005') { throw 'Start requires the exact Week 2 development confirmation.' }
    ...
    if ($recheckedHistory.Count -ne 2 -or
        (@($recheckedHistory.metadata.name | Sort-Object) -join ',') -cne
        (@($history.metadata.name | Sort-Object) -join ',')) {
    ...
    Write-RecoveryMarker -Marker @{
        ...
        previousExecutions = @($history.metadata.name); backupId = [string]$backup.id
