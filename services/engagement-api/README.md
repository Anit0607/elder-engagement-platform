...
The current Cloud Run configuration is deliberately development-only. It supplies the complete non-secret startup contract and uses separate lightweight startup/liveness probes. EE-052 wires `/ready` to read-only database, sign-in-limit-table, storage-object and Google phone-certificate checks. The reviewed image was deployed in the development project; the protected public address returned HTTP 200 on repeat readiness checks, while unsigned private requests remained denied. See `../../api/evidence/ee-052-readiness-development.md`. A local instance without connected runtime still returns the intentionally fail-closed 503 response.
